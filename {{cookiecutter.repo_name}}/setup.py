...
import os
{%- if cookiecutter._c_extension_support == 'yes' %}
import platform
{%- endif -%}
{%- endif -%}
...
{%- if cookiecutter._c_extension_support not in ['no', 'cffi'] %}
...
{%- if cookiecutter._c_extension_support == 'yes' %} and platform.system() == 'Linux'{% endif %}:
{%- if cookiecutter._c_extension_support == 'cython' %}
    CFLAGS = os.environ['CFLAGS'] = '-DCYTHON_TRACE=1'
    LFLAGS = os.environ['LFLAGS'] = ''
{%- elif cookiecutter._c_extension_support == 'yes' %}
...
class OptionalBuildExt(build_ext):
    ...
    def run(self):
        try:
            ...
            {% if cookiecutter._legacy_python == "yes" %}build_ext.run(self){% else %}super().run(){% endif %}
...
class BinaryDistribution(Distribution):
    ...
    def has_ext_modules(self):
        return super({% if cookiecutter._legacy_python == "yes" %}BinaryDistribution, self{% endif %}).has_ext_modules() or not os.environ.get('SETUPPY_ALLOW_PURE')
...
{%- if cookiecutter._setup_py_uses_setuptools_scm == 'yes' %}
    use_scm_version={
        ...
        'fallback_version': '{{ cookiecutter._version }}',
    },
{%- else %}
...
{%- endif %}
    ...
    author={{ '{0!r}'.format(cookiecutter._full_name).lstrip('ub') }},
    author_email={{ '{0!r}'.format(cookiecutter._email).lstrip('ub') }},
{%- if cookiecutter._repo_hosting_domain == "no" %}
    url='file://' + os.path.abspath(dirname(__file__)),
{%- else %}
    url='https://{{ cookiecutter._repo_hosting_domain }}/{{ cookiecutter._repo_username }}/{{ cookiecutter.repo_name }}',
{%- endif %}
...
{%- if cookiecutter._license == "no" %}
{%- elif cookiecutter._license in ["BSD 2-Clause License", "BSD 3-Clause License"] %}
        'License :: OSI Approved :: BSD License',
{%- elif cookiecutter._license == "MIT license" %}
        'License :: OSI Approved :: MIT License',
{%- elif cookiecutter._license == "ISC license" %}
        'License :: OSI Approved :: ISC License (ISCL)',
{%- elif cookiecutter._license == "Apache Software License 2.0" %}
        'License :: OSI Approved :: Apache Software License',
{%- elif 'LGPLv3+' in cookiecutter._license %}
        'License :: OSI Approved :: GNU Lesser General Public License v3 or later (LGPLv3+)'
{%- elif 'LGPLv3' in cookiecutter._license %}
        'License :: OSI Approved :: GNU Lesser General Public License v3 (LGPLv3)'
{%- elif 'LGPLv2' in cookiecutter._license %}
...
{%- if cookiecutter._legacy_python == "no" %}
...
{%- endif %}
        ...
        'Topic :: Utilities',
{%- if cookiecutter._pypi_disable_upload == "yes" %}
...
{%- if cookiecutter._sphinx_docs == "yes" %}
        'Documentation': '{{ cookiecutter._sphinx_docs_hosting }}',
        'Changelog': '{{ cookiecutter._sphinx_docs_hosting }}en/latest/changelog.html',
{%- else %}
        'Changelog': 'https://{{ cookiecutter._repo_hosting_domain }}/{{ cookiecutter._repo_username }}/{{ cookiecutter.repo_name }}/blob/master/CHANGELOG.rst',
{%- endif %}
        'Issue Tracker': 'https://{{ cookiecutter._repo_hosting_domain }}/{{ cookiecutter._repo_username }}/{{ cookiecutter.repo_name }}/issues',
    },
{%- endif %}
    keywords=[
...
{%- if cookiecutter._setup_py_uses_pytest_runner == 'yes' %}
        'pytest-runner',{% endif %}
{%- if cookiecutter._setup_py_uses_setuptools_scm == 'yes' %}
        'setuptools_scm>=3.3.1{% if cookiecutter._legacy_python == 'yes' %},<6.0{% endif %}',{% endif %}
{%- endset %}
{%- if cookiecutter._c_extension_support == 'cython' %}
...
{%- if cookiecutter._c_extension_support == 'cython' %} '*.pyx' if Cython else '*.c'
{%- else %} '*.c'{% endif %}))
    ],
